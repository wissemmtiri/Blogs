...
<img src="./images/git/project_arch.png" alt="project architecture" style="height:70vh;">

A git project, can be modelized as a **Linked list** (<a href="https://fr.wikipedia.org/wiki/Liste_cha%C3%AEn%C3%A9e">Read more about **Linked Lists**</a>), where nodes are represented by **commits**, and the head of list is called **HEAD**.

- **HEAD:** is the pointer to the last commit, so a pointer to the whole project. _(Think of moving the head pointer to remove unwanted commits 🧐)_

- **COMMIT:** a commit is a snapshot in time of your project. It captures the state of all your staged files and directories, along with metadata such as commit message, author, date, and **commit hash** (unique id of the commit)
